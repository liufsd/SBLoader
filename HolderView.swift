...
class HolderView: UIView {
  ...
  required init(coder: NSCoder) {
    ...
    func wobbleOval(){
        //1
        layer.addSublayer(triangleLayer) // TriangleLayer class SubLayer
        ovalLayer.wobble()
    ...
    //2
        var rotationAnimation : CABasicAnimation = CABasicAnimation(keyPath: "path")
        rotationAnimation.fromValue = CGFloat(M_PI * 2.0)
  ...
  NSTimer.scheduledTimerWithTimeInterval(0.90, target: self, selector: "expandView", userInfo: nil, repeats: false)
    ...
    func expandView(){
     //1 
        ...
        //2
        frame = CGRectMake(
        frame.origin.x - blueRectangleLayer.lineWidth,
